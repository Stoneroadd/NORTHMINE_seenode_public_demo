...
async def _run_investigation(live: LiveSession, user: dict, plan: InvestigationPlan, correlation_id: str, ip: str) -> None:
    ...
    live.session = live.session.model_copy(update={"active_investigation_id": None})
    # C7: a diferencia del camino cancelado (que si volvia a IDLE mas abajo
    ...
    # el turno explicitamente a idle es lo que ya hacia la seccion 8 del
    # brief para el resto de los flujos.
    await _set_state(live, AgentRuntimeState.IDLE, correlation_id=correlation_id)
...
async def cancel_investigation(live: LiveSession, user: dict, correlation_id: str, ip: str) -> None:
    ...
    if live.current_investigation_task is None:
        ...
        # (InvalidStateTransition) y, antes de este chequeo, esa excepcion
        # se propagaba sin capturar por todo _dispatch_client_event hasta
        # el loop del WebSocket. Se usa can_transition en vez de comparar
        # contra un unico estado "seguro" (ej. solo IDLE) porque no es el
        # unico estado sin arista hacia cancelled - ver el fix de arriba en
        # el camino exitoso de _run_investigation.
