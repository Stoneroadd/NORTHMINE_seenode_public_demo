...
import { Activity, AlertTriangle, Database, RadioTower, Server, ShieldCheck, type LucideIcon } from 'lucide-react'
...
export function SystemPage() {
  ...
  const availability = health?.status === 'ok' ? t.conectividad_si : t.verificando
  ...
  return (
    <section className="page-content">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 12 }}>
        <Metric icon={RadioTower} label={t.metric_backend} value={availability} />
        <Metric icon={ShieldCheck} label={t.metric_version} value={health?.production_ready ? t.conectividad_si : t.verificando} />
        <Metric icon={Database} label={t.metric_base_datos} value={health?.database ? t.conectividad_si : t.verificando} />
        <Metric icon={Server} label={t.metric_usuarios} value={health?.identity_store ? t.conectividad_api_protegida : t.verificando} />
        <Metric icon={Activity} label={t.metric_entorno} value={t.conectividad_local} />
      </div>
      ...
      <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 12 }}>
        <div className="command-card">
          ...
          <p>{t.conectividad_estado}: {availability}</p>
      ...
      <div className="command-card" style={{ marginTop: 18 }}>
        ...
        <p>{recentErrors.length ? `${recentErrors.length} incidencias recientes están disponibles en observabilidad segura.` : t.sin_errores}</p>
      </div>
    </section>
  )
}
