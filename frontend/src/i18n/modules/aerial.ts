...
export const aerialT: ModuleDict<AerialT> = {
  ...
  de: {
    viewer_kicker: 'Viewer',
    viewer_title: (fileName) => `Orthomosaik ${fileName}`,
    viewer_hint: 'Mausrad: Zoom / Ziehen: Verschieben / Doppelklick: Vergrößern',
    zoom_out_aria: 'Verkleinern',
    zoom_in_aria: 'Vergrößern',
    generating_preview: 'TIF-Vorschau wird erzeugt... Beim ersten Mal kann dies einige Sekunden dauern.',
    preview_error: 'Die Vorschau konnte nicht erzeugt werden. Bitte erneut versuchen und die Seite neu laden.',
    orthomosaic_alt: (fileName) => `Orthomosaik ${fileName}`,

    loading_label: 'Luftansichtsstatus wird geladen...',
    error_detail: 'Das Modul Luftansicht konnte nicht geladen werden.',
    eyebrow: 'Luftansicht',
    title: 'Orthomosaike und operative Abdeckung',
    description: 'Echte Flug-Orthomosaike. Der Viewer zeigt die JPEG-Vorschau (3000 px); das originale TIF bleibt auf dem Server.',
    drive_connected: 'Flug-Drive: verbunden',
    drive_not_configured: 'Drive nicht konfiguriert',
    auto_sync: (intervalMin, lastRunSuffix) => `Automatisch alle ${intervalMin} min${lastRunSuffix}`,
    auto_sync_last_run_suffix: (time) => ` / zuletzt ${time}`,
    syncing: 'Wird synchronisiert...',
    sync_button: 'Orthomosaik suchen',
    drive_error_default: 'Fehler beim Prüfen des Drive-Ordners.',
    files_downloaded: (count, names) => `${count} Datei(en) heruntergeladen: ${names}`,
    no_new_files: (detail) => `Keine neuen Orthomosaike im Drive-Ordner${detail}.`,
    checked_suffix: (folder) => ` (${folder})`,
    ...
    kpi_files_trend_none: 'keine Datei',
    kpi_caex_title: 'CAEX',
    kpi_caex_subtitle: 'Operative Abdeckung',
    kpi_caex_trend: (loaders) => `${loaders} Lader`,
    kpi_last_title: 'Letzter',
    kpi_last_subtitle_none: 'Kein Orthomosaik',
    kpi_last_trend_validating: 'in Validierung',
    tif_loaded: 'TIF geladen',
    tif_not_loaded: 'TIF nicht geladen',

    orthomosaic_kicker: 'Orthomosaik',
    last_file_available: 'Letzte verfügbare Datei',
    file_label: 'Datei',
    format_label: 'Format',
    size_label: 'Größe',
    date_label: 'Datum',
    heavy_viewer_hint: 'Schwerer Viewer in Validierung. Aktueller Status: metadata-only.',
    no_operational_data: 'Nicht genügend Daten für die operative Bewertung',
